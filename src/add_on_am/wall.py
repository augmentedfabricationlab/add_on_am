...
class Wall(object):
    ...
    def create_quad_mesh(self, inputBrep, max_edge_length=0.05, min_edge_length=0.05, target_edge_length=0.05):
        """Creates a compas quad mesh from an input Brep surface

        Args:
        density (int): Density of the mesh.
        inputBrep (:obj :class:`Rhino.Geometry.Brep`)
        """

        # Create a compas mesh from an input Brep surface
        par = rg.MeshingParameters()
        #par.GridMinCount = density*10
        par.MinimumEdgeLength = 0.05
        par.MaximumEdgeLength = 0.05
