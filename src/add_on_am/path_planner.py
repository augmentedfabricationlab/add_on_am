...
class SurfacePathPlanner():
    ...
    def heat_method(self, length, height, crv_length, direction="x"):
        ...
        # distance from the right edge does not properly work, idk why, 
        # but inversing the left edge distance should be fine for now
        start = []
        end = []
        if direction == "x":
    ...
    def find_crossings(self, vertices, dists):
        ...
        # if not (all(d > 0 for d in dists) or all(d < 0 for d in dists)):
        #     continue
