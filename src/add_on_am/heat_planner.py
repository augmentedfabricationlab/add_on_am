...
class HeatPathPlanner():
    """HeatPathPlanner class for generating path based on heat method."""
    def __init__(self):
        self.mesh = None
    ...
    def heat_method(self, length, height, crv_length, start=[], end=[], direction="x"):
        """Heat method for generating path."""
        if self.mesh == None:
            raise ValueError
        tri = False
        ...
        if direction == "x" and not start and not end:
            start = []
            end = []
        ...
        d_max_start = distances_start[end[len(end)//2]]
        d_max_end = distances_end[start[len(start)//2]]
        ...
        N = int(avg_dist / avg_thickness)//2
        # N = 100
        ...
        for i, weight in enumerate(d_weight):
            for face in self.mesh.faces():
                ...
                if tri:
                    crossings = self.find_crossings_tri(vertices, dists)
                else:
                    crossings = self.find_crossings_quad(vertices, dists)
                if crossings != []:
                    zero_crossings[i].extend(crossings)
        
        self.isolines_spherical(self.mesh.vertex_coordinates(start[len(start)//2]), self.mesh.vertex_coordinates(end[len(end)//2]), zero_crossings)
    ...
    def barycentric(self, start, triangle, nodes):
        """find the barycentric coordinates of the points of the isoline"""
        index = 0
    ...
    def spherical_factor(self, x, y, z, origin, result="theta"):
        """calculate the spherical coordinate of a point relative to a new origin"""
    ...
    def find_crossings(self, vertices, dists):
        """find the zero crossings of a line"""
        [a, b, c, d] = dists
    ...
    def find_crossings_tri(self, vertices, dists):
        """find the zero crossings of a line"""
        [a, b, c] = dists
        ...
        if math.copysign(1, b) !=  math.copysign(1, c):
            zero_crossing.append(self.calculate_crossings(b, c, v_b, v_c))
        if math.copysign(1, c) !=  math.copysign(1, a):
            zero_crossing.append(self.calculate_crossings(c, a, v_c, v_a))
        return zero_crossing

    
    def calculate_crossings(self, start, end, start_vertice, end_vertice):
        """calculate the location of the zero crossing between two points"""
    ...
    def connect_isolines(self, start, nodes):
        """connect the isolines to form a network in a zig-zag pattern"""
        index = 0
        for i, isoline_nodes in enumerate(nodes):
            if len(isoline_nodes) == 0:
                print(index)
